...
class clanguages_list extends clanguages {
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "flanguageslistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.flanguageslist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"flanguageslistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
...
// Validate form
flanguageslist.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		if (checkrow) {
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $languages->name_ar->FldCaption(), $languages->name_ar->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_name_en");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $languages->name_en->FldCaption(), $languages->name_en->ReqErrMsg)) ?>");
...
// Check empty row
flanguageslist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
flanguageslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
if ($languages->CurrentAction == "gridadd") {
	$languages->CurrentFilter = "0=1";
	$languages_list->StartRec = 1;
	$languages_list->DisplayRecs = $languages->GridAddRowCount;
	$languages_list->TotalRecs = $languages_list->DisplayRecs;
	$languages_list->StopRec = $languages_list->DisplayRecs;
} else {
	$bSelectLimit = $languages_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($languages_list->TotalRecs <= 0)
			$languages_list->TotalRecs = $languages->ListRecordCount();
	} else {
	...
	$languages_list->StartRec = 1;
	if ($languages_list->DisplayRecs <= 0 || ($languages->Export <> "" && $languages->ExportAll)) // Display all records
		$languages_list->DisplayRecs = $languages_list->TotalRecs;
	if (!($languages->Export <> "" && $languages->ExportAll))
...
<form name="flanguageslistsrch" id="flanguageslistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
...
<div class="box ewBox ewGrid<?php if ($languages_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> languages">
<?php if ($languages->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($languages->CurrentAction <> "gridadd" && $languages->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($languages_list->Pager)) $languages_list->Pager = new cPrevNextPager($languages_list->StartRec, $languages_list->DisplayRecs, $languages_list->TotalRecs, $languages_list->AutoHidePager) ?>
<?php if ($languages_list->Pager->RecordCount > 0 && $languages_list->Pager->Visible) { ?>
<div class="ewPager">
...
<?php } ?>
<input type="hidden" name="t" value="languages">
<div id="gmp_languages" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($languages_list->TotalRecs > 0 || $languages->CurrentAction == "gridedit") { ?>
...
// Header row
$languages_list->RowType = EW_ROWTYPE_HEADER;
...
?>
<?php if ($languages->id->Visible) { // id ?>
	<?php if ($languages->SortUrl($languages->id) == "") { ?>
		<th data-name="id" class="<?php echo $languages->id->HeaderCellClass() ?>"><div id="elh_languages_id" class="languages_id"><div class="ewTableHeaderCaption"><?php echo $languages->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id" class="<?php echo $languages->id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $languages->SortUrl($languages->id) ?>',1);"><div id="elh_languages_id" class="languages_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $languages->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($languages->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($languages->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($languages->name_ar->Visible) { // name_ar ?>
	<?php if ($languages->SortUrl($languages->name_ar) == "") { ?>
		<th data-name="name_ar" class="<?php echo $languages->name_ar->HeaderCellClass() ?>"><div id="elh_languages_name_ar" class="languages_name_ar"><div class="ewTableHeaderCaption"><?php echo $languages->name_ar->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name_ar" class="<?php echo $languages->name_ar->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $languages->SortUrl($languages->name_ar) ?>',1);"><div id="elh_languages_name_ar" class="languages_name_ar">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $languages->name_ar->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($languages->name_ar->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($languages->name_ar->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($languages->name_en->Visible) { // name_en ?>
	<?php if ($languages->SortUrl($languages->name_en) == "") { ?>
		<th data-name="name_en" class="<?php echo $languages->name_en->HeaderCellClass() ?>"><div id="elh_languages_name_en" class="languages_name_en"><div class="ewTableHeaderCaption"><?php echo $languages->name_en->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name_en" class="<?php echo $languages->name_en->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $languages->SortUrl($languages->name_en) ?>',1);"><div id="elh_languages_name_en" class="languages_name_en">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $languages->name_en->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($languages->name_en->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($languages->name_en->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
<tbody>
<?php
if ($languages->ExportAll && $languages->Export <> "") {
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($languages_list->FormKeyCountName) && ($languages->CurrentAction == "gridadd" || $languages->CurrentAction == "gridedit" || $languages->CurrentAction == "F")) {
		$languages_list->KeyCount = $objForm->GetValue($languages_list->FormKeyCountName);
		$languages_list->StopRec = $languages_list->StartRec + $languages_list->KeyCount - 1;
	}
}
...
if ($languages->CurrentAction == "gridadd")
	$languages_list->RowIndex = 0;
if ($languages->CurrentAction == "gridedit")
	$languages_list->RowIndex = 0;
while ($languages_list->RecCnt < $languages_list->StopRec) {
	...
	if (intval($languages_list->RecCnt) >= intval($languages_list->StartRec)) {
		...
		if ($languages->CurrentAction == "gridadd" || $languages->CurrentAction == "gridedit" || $languages->CurrentAction == "F") {
			$languages_list->RowIndex++;
			$objForm->Index = $languages_list->RowIndex;
			if ($objForm->HasValue($languages_list->FormActionName))
				$languages_list->RowAction = strval($objForm->GetValue($languages_list->FormActionName));
			elseif ($languages->CurrentAction == "gridadd")
				$languages_list->RowAction = "insert";
			else
				$languages_list->RowAction = "";
		}
		...
		$languages->CssClass = "";
		if ($languages->CurrentAction == "gridadd") {
			$languages_list->LoadRowValues(); // Load default values
		} else {
			$languages_list->LoadRowValues($languages_list->Recordset); // Load row values
		}
		$languages->RowType = EW_ROWTYPE_VIEW; // Render view
		if ($languages->CurrentAction == "gridadd") // Grid add
			$languages->RowType = EW_ROWTYPE_ADD; // Render add
		if ($languages->CurrentAction == "gridadd" && $languages->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$languages_list->RestoreCurrentRowFormValues($languages_list->RowIndex); // Restore form values
		if ($languages->CurrentAction == "gridedit") { // Grid edit
			if ($languages->EventCancelled) {
				$languages_list->RestoreCurrentRowFormValues($languages_list->RowIndex); // Restore form values
			}
			if ($languages_list->RowAction == "insert")
				$languages->RowType = EW_ROWTYPE_ADD; // Render add
			else
				$languages->RowType = EW_ROWTYPE_EDIT; // Render edit
		}
		if ($languages->CurrentAction == "gridedit" && ($languages->RowType == EW_ROWTYPE_EDIT || $languages->RowType == EW_ROWTYPE_ADD) && $languages->EventCancelled) // Update failed
			$languages_list->RestoreCurrentRowFormValues($languages_list->RowIndex); // Restore form values
		if ($languages->RowType == EW_ROWTYPE_EDIT) // Edit row
			$languages_list->EditRowCnt++;
		...
		if ($languages_list->RowAction <> "delete" && $languages_list->RowAction <> "insertdelete" && !($languages_list->RowAction == "insert" && $languages->CurrentAction == "F" && $languages_list->EmptyRow())) {
?>
...
?>
	<?php if ($languages->id->Visible) { // id ?>
		<td data-name="id"<?php echo $languages->id->CellAttributes() ?>>
<?php if ($languages->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="languages" data-field="x_id" name="o<?php echo $languages_list->RowIndex ?>_id" id="o<?php echo $languages_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($languages->id->OldValue) ?>">
<?php } ?>
<?php if ($languages->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_id" class="form-group languages_id">
<span<?php echo $languages->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $languages->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="languages" data-field="x_id" name="x<?php echo $languages_list->RowIndex ?>_id" id="x<?php echo $languages_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($languages->id->CurrentValue) ?>">
<?php } ?>
<?php if ($languages->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_id" class="languages_id">
<span<?php echo $languages->id->ViewAttributes() ?>>
<?php echo $languages->id->ListViewValue() ?></span>
</span>
<?php } ?>
</td>
	<?php } ?>
	<?php if ($languages->name_ar->Visible) { // name_ar ?>
		<td data-name="name_ar"<?php echo $languages->name_ar->CellAttributes() ?>>
<?php if ($languages->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_name_ar" class="form-group languages_name_ar">
<input type="text" data-table="languages" data-field="x_name_ar" name="x<?php echo $languages_list->RowIndex ?>_name_ar" id="x<?php echo $languages_list->RowIndex ?>_name_ar" placeholder="<?php echo ew_HtmlEncode($languages->name_ar->getPlaceHolder()) ?>" value="<?php echo $languages->name_ar->EditValue ?>"<?php echo $languages->name_ar->EditAttributes() ?>>
</span>
<input type="hidden" data-table="languages" data-field="x_name_ar" name="o<?php echo $languages_list->RowIndex ?>_name_ar" id="o<?php echo $languages_list->RowIndex ?>_name_ar" value="<?php echo ew_HtmlEncode($languages->name_ar->OldValue) ?>">
<?php } ?>
<?php if ($languages->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_name_ar" class="form-group languages_name_ar">
<input type="text" data-table="languages" data-field="x_name_ar" name="x<?php echo $languages_list->RowIndex ?>_name_ar" id="x<?php echo $languages_list->RowIndex ?>_name_ar" placeholder="<?php echo ew_HtmlEncode($languages->name_ar->getPlaceHolder()) ?>" value="<?php echo $languages->name_ar->EditValue ?>"<?php echo $languages->name_ar->EditAttributes() ?>>
...
<?php } ?>
<?php if ($languages->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_name_ar" class="languages_name_ar">
<span<?php echo $languages->name_ar->ViewAttributes() ?>>
<?php echo $languages->name_ar->ListViewValue() ?></span>
</span>
<?php } ?>
</td>
	<?php } ?>
	<?php if ($languages->name_en->Visible) { // name_en ?>
		<td data-name="name_en"<?php echo $languages->name_en->CellAttributes() ?>>
<?php if ($languages->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_name_en" class="form-group languages_name_en">
<input type="text" data-table="languages" data-field="x_name_en" name="x<?php echo $languages_list->RowIndex ?>_name_en" id="x<?php echo $languages_list->RowIndex ?>_name_en" placeholder="<?php echo ew_HtmlEncode($languages->name_en->getPlaceHolder()) ?>" value="<?php echo $languages->name_en->EditValue ?>"<?php echo $languages->name_en->EditAttributes() ?>>
</span>
<input type="hidden" data-table="languages" data-field="x_name_en" name="o<?php echo $languages_list->RowIndex ?>_name_en" id="o<?php echo $languages_list->RowIndex ?>_name_en" value="<?php echo ew_HtmlEncode($languages->name_en->OldValue) ?>">
<?php } ?>
<?php if ($languages->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_name_en" class="form-group languages_name_en">
<input type="text" data-table="languages" data-field="x_name_en" name="x<?php echo $languages_list->RowIndex ?>_name_en" id="x<?php echo $languages_list->RowIndex ?>_name_en" placeholder="<?php echo ew_HtmlEncode($languages->name_en->getPlaceHolder()) ?>" value="<?php echo $languages->name_en->EditValue ?>"<?php echo $languages->name_en->EditAttributes() ?>>
...
<?php } ?>
<?php if ($languages->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $languages_list->RowCnt ?>_languages_name_en" class="languages_name_en">
<span<?php echo $languages->name_en->ViewAttributes() ?>>
<?php echo $languages->name_en->ListViewValue() ?></span>
</span>
<?php } ?>
</td>
	<?php } ?>
...
<?php if ($languages->RowType == EW_ROWTYPE_ADD || $languages->RowType == EW_ROWTYPE_EDIT) { ?>
<script type="text/javascript">
flanguageslist.UpdateOpts(<?php echo $languages_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($languages->CurrentAction <> "gridadd")
		if (!$languages_list->Recordset->EOF) $languages_list->Recordset->MoveNext();
}
?>
<?php
	if ($languages->CurrentAction == "gridadd" || $languages->CurrentAction == "gridedit") {
		$languages_list->RowIndex = '$rowindex$';
		$languages_list->LoadRowValues();

		// Set row properties
		$languages->ResetAttrs();
		$languages->RowAttrs = array_merge($languages->RowAttrs, array('data-rowindex'=>$languages_list->RowIndex, 'id'=>'r0_languages', 'data-rowtype'=>EW_ROWTYPE_ADD));
		ew_AppendClass($languages->RowAttrs["class"], "ewTemplate");
		$languages->RowType = EW_ROWTYPE_ADD;
		...
		// Render list options
		$languages_list->RenderListOptions();
		$languages_list->StartRowCnt = 0;
?>
	<tr<?php echo $languages->RowAttributes() ?>>
<?php

// Render list options (body, left)
$languages_list->ListOptions->Render("body", "left", $languages_list->RowIndex);
?>
	<?php if ($languages->id->Visible) { // id ?>
		<td data-name="id">
<input type="hidden" data-table="languages" data-field="x_id" name="o<?php echo $languages_list->RowIndex ?>_id" id="o<?php echo $languages_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($languages->id->OldValue) ?>">
...
<span id="el$rowindex$_languages_name_ar" class="form-group languages_name_ar">
<input type="text" data-table="languages" data-field="x_name_ar" name="x<?php echo $languages_list->RowIndex ?>_name_ar" id="x<?php echo $languages_list->RowIndex ?>_name_ar" placeholder="<?php echo ew_HtmlEncode($languages->name_ar->getPlaceHolder()) ?>" value="<?php echo $languages->name_ar->EditValue ?>"<?php echo $languages->name_ar->EditAttributes() ?>>
</span>
<input type="hidden" data-table="languages" data-field="x_name_ar" name="o<?php echo $languages_list->RowIndex ?>_name_ar" id="o<?php echo $languages_list->RowIndex ?>_name_ar" value="<?php echo ew_HtmlEncode($languages->name_ar->OldValue) ?>">
</td>
...
<span id="el$rowindex$_languages_name_en" class="form-group languages_name_en">
<input type="text" data-table="languages" data-field="x_name_en" name="x<?php echo $languages_list->RowIndex ?>_name_en" id="x<?php echo $languages_list->RowIndex ?>_name_en" placeholder="<?php echo ew_HtmlEncode($languages->name_en->getPlaceHolder()) ?>" value="<?php echo $languages->name_en->EditValue ?>"<?php echo $languages->name_en->EditAttributes() ?>>
</span>
<input type="hidden" data-table="languages" data-field="x_name_en" name="o<?php echo $languages_list->RowIndex ?>_name_en" id="o<?php echo $languages_list->RowIndex ?>_name_en" value="<?php echo ew_HtmlEncode($languages->name_en->OldValue) ?>">
</td>
...
// Render list options (body, right)
$languages_list->ListOptions->Render("body", "right", $languages_list->RowIndex);
?>
<script type="text/javascript">
flanguageslist.UpdateOpts(<?php echo $languages_list->RowIndex ?>);
</script>
...
<?php } ?>
<?php if ($languages->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $languages_list->FormKeyCountName ?>" id="<?php echo $languages_list->FormKeyCountName ?>" value="<?php echo $languages_list->KeyCount ?>">
<?php echo $languages_list->MultiSelectKey ?>
<?php } ?>
<?php if ($languages->CurrentAction == "gridedit") { ?>
