...
class cstudents extends cTable {
	...
	function __construct() {
		...
		// user_id
		$this->user_id = new cField('students', 'students', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 19, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_id->Sortable = TRUE; // Allow sort
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// is_parent
		$this->is_parent = new cField('students', 'students', 'x_is_parent', 'is_parent', '`is_parent`', '`is_parent`', 16, -1, FALSE, '`is_parent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->is_parent->Sortable = TRUE; // Allow sort
		$this->is_parent->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['is_parent'] = &$this->is_parent;

		// level_id
		$this->level_id = new cField('students', 'students', 'x_level_id', 'level_id', '`level_id`', '`level_id`', 19, -1, FALSE, '`level_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->level_id->Sortable = TRUE; // Allow sort
		$this->level_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['level_id'] = &$this->level_id;

		// study_year
		$this->study_year = new cField('students', 'students', 'x_study_year', 'study_year', '`study_year`', '`study_year`', 202, -1, FALSE, '`study_year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->study_year->Sortable = TRUE; // Allow sort
		$this->study_year->OptionCount = 5;
		$this->fields['study_year'] = &$this->study_year;

		// language_id
		$this->language_id = new cField('students', 'students', 'x_language_id', 'language_id', '`language_id`', '`language_id`', 19, -1, FALSE, '`language_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->language_id->Sortable = TRUE; // Allow sort
		$this->language_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['language_id'] = &$this->language_id;

		// langauge_level_id
		$this->langauge_level_id = new cField('students', 'students', 'x_langauge_level_id', 'langauge_level_id', '`langauge_level_id`', '`langauge_level_id`', 19, -1, FALSE, '`langauge_level_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->langauge_level_id->Sortable = TRUE; // Allow sort
		$this->langauge_level_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['langauge_level_id'] = &$this->langauge_level_id;

		// created_at
		$this->created_at = new cField('students', 'students', 'x_created_at', 'created_at', '`created_at`', ew_CastDateFieldForLike('`created_at`', 0, "DB"), 135, 0, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->Sortable = TRUE; // Allow sort
		$this->created_at->FldDefaultErrMsg = str_replace("%s", $GLOBALS["EW_DATE_FORMAT"], $Language->Phrase("IncorrectDate"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('students', 'students', 'x_updated_at', 'updated_at', '`updated_at`', ew_CastDateFieldForLike('`updated_at`', 0, "DB"), 135, 0, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function LoadListRowValues(&$rs) {
		...
		$this->is_parent->setDbValue($rs->fields('is_parent'));
		$this->level_id->setDbValue($rs->fields('level_id'));
		$this->study_year->setDbValue($rs->fields('study_year'));
		$this->language_id->setDbValue($rs->fields('language_id'));
		$this->langauge_level_id->setDbValue($rs->fields('langauge_level_id'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->is_parent->Exportable) $Doc->ExportCaption($this->is_parent);
					if ($this->level_id->Exportable) $Doc->ExportCaption($this->level_id);
					if ($this->study_year->Exportable) $Doc->ExportCaption($this->study_year);
					if ($this->language_id->Exportable) $Doc->ExportCaption($this->language_id);
					if ($this->langauge_level_id->Exportable) $Doc->ExportCaption($this->langauge_level_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->is_parent->Exportable) $Doc->ExportField($this->is_parent);
						if ($this->level_id->Exportable) $Doc->ExportField($this->level_id);
						if ($this->study_year->Exportable) $Doc->ExportField($this->study_year);
						if ($this->language_id->Exportable) $Doc->ExportField($this->language_id);
						if ($this->langauge_level_id->Exportable) $Doc->ExportField($this->langauge_level_id);
