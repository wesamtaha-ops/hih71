...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fteachers_packagesedit" id="fteachers_packagesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { teachers_packages: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fteachers_packagesedit;
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_teachers_packages_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_teachers_packages_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="teachers_packages" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_teachers_packages_teacher_id">
    <select
        ...
        data-select2-id="fteachers_packagesedit_x_teacher_id"
        <?php } ?>
...
<?php if (!$Page->teacher_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fteachers_packagesedit", function() {
    var options = { name: "x_teacher_id", selectId: "fteachers_packagesedit_x_teacher_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fteachers_packagesedit.lists.teacher_id?.lookupOptions.length) {
        options.data = { id: "x_teacher_id", form: "fteachers_packagesedit" };
    } else {
        options.ajax = { id: "x_teacher_id", form: "fteachers_packagesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fteachers_packagesedit", function() {
    fteachers_packagesedit.createAutoSuggest(Object.assign({"id":"x_currency_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->currency_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.teachers_packages.fields.currency_id.autoSuggestOptions));
});
</script>
...
<?php if ($Page->created_at->Visible) { // created_at ?>
    <div id="r_created_at"<?= $Page->created_at->rowAttributes() ?>>
        <label id="elh_teachers_packages_created_at" for="x_created_at" class="<?= $Page->LeftColumnClass ?>"><?= $Page->created_at->caption() ?><?= $Page->created_at->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->created_at->cellAttributes() ?>>
<span id="el_teachers_packages_created_at">
<input type="<?= $Page->created_at->getInputTextType() ?>" name="x_created_at" id="x_created_at" data-table="teachers_packages" data-field="x_created_at" value="<?= $Page->created_at->EditValue ?>" placeholder="<?= HtmlEncode($Page->created_at->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->created_at->formatPattern()) ?>"<?= $Page->created_at->editAttributes() ?> aria-describedby="x_created_at_help">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fteachers_packagesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fteachers_packagesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
