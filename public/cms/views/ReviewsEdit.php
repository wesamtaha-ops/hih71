...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="freviewsedit" id="freviewsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_reviews_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_reviews_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="reviews" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_reviews_from_user_id">
    <select
        ...
        data-select2-id="freviewsedit_x_from_user_id"
        <?php } ?>
        data-table="reviews"
...
<?php if (!$Page->from_user_id->IsNativeSelect) { ?>
<script>
loadjs.ready("freviewsedit", function() {
    var options = { name: "x_from_user_id", selectId: "freviewsedit_x_from_user_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (freviewsedit.lists.from_user_id?.lookupOptions.length) {
        options.data = { id: "x_from_user_id", form: "freviewsedit" };
    } else {
        options.ajax = { id: "x_from_user_id", form: "freviewsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("freviewsedit", function() {
    freviewsedit.createAutoSuggest(Object.assign({"id":"x_to_user_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->to_user_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.reviews.fields.to_user_id.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["freviewsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("freviewsedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["freviewsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("freviewsedit", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="freviewsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="freviewsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
