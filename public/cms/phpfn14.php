...
function CurrentProjectID() {
	...
	return "{D43A73A4-5F37-4161-A00D-2E65107145C9}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "") {
		...
		if (!$valid) {
			if (EW_CASE_SENSITIVE_PASSWORD) {
				$valid = (!$customValid && EW_ADMIN_USER_NAME == $usr && EW_ADMIN_PASSWORD == $pwd) ||
					($customValid && EW_ADMIN_USER_NAME == $usr);
			} else {
				$valid = (!$customValid && ew_SameText(EW_ADMIN_USER_NAME, $usr) && ew_SameText(EW_ADMIN_PASSWORD, $pwd)) ||
					($customValid && ew_SameText(EW_ADMIN_USER_NAME, $usr));
			}
			if ($valid) {
				$_SESSION[EW_SESSION_STATUS] = "login";
