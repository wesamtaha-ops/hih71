...
//
class ccurrencies extends cTable {
	var $id;
	var $name_ar;
	var $name_en;
	var $rate;
	...
	function __construct() {
		...
		// id
		$this->id = new cField('currencies', 'currencies', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// name_ar
		$this->name_ar = new cField('currencies', 'currencies', 'x_name_ar', 'name_ar', '`name_ar`', '`name_ar`', 201, -1, FALSE, '`name_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->name_ar->Sortable = TRUE; // Allow sort
		$this->fields['name_ar'] = &$this->name_ar;

		// name_en
		$this->name_en = new cField('currencies', 'currencies', 'x_name_en', 'name_en', '`name_en`', '`name_en`', 201, -1, FALSE, '`name_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->name_en->Sortable = TRUE; // Allow sort
		$this->fields['name_en'] = &$this->name_en;

		// rate
		$this->rate = new cField('currencies', 'currencies', 'x_rate', 'rate', '`rate`', '`rate`', 5, -1, FALSE, '`rate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rate->Sortable = TRUE; // Allow sort
		$this->rate->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['rate'] = &$this->rate;

		// symbol
		$this->symbol = new cField('currencies', 'currencies', 'x_symbol', 'symbol', '`symbol`', '`symbol`', 200, -1, FALSE, '`symbol`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->symbol->Sortable = TRUE; // Allow sort
		$this->fields['symbol'] = &$this->symbol;

		// created_at
		$this->created_at = new cField('currencies', 'currencies', 'x_created_at', 'created_at', '`created_at`', ew_CastDateFieldForLike('`created_at`', 0, "DB"), 135, 0, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// updated_at
		$this->updated_at = new cField('currencies', 'currencies', 'x_updated_at', 'updated_at', '`updated_at`', ew_CastDateFieldForLike('`updated_at`', 0, "DB"), 135, 0, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`currencies`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "currenciesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "currenciesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("currenciesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("currenciesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "currenciesadd.php?" . $this->UrlParm($parm);
		else
			$url = "currenciesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("currenciesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("currenciesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("currenciesdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->symbol->setDbValue($rs->fields('symbol'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->symbol->Exportable) $Doc->ExportCaption($this->symbol);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->name_ar->Exportable) $Doc->ExportCaption($this->name_ar);
					if ($this->name_en->Exportable) $Doc->ExportCaption($this->name_en);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->symbol->Exportable) $Doc->ExportField($this->symbol);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->name_ar->Exportable) $Doc->ExportField($this->name_ar);
						if ($this->name_en->Exportable) $Doc->ExportField($this->name_en);
