...
$topics_delete = NULL; // Initialize page object first

class ctopics_delete extends ctopics {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("topicslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in topics class, topicsinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("topicslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($topics_delete)) $topics_delete = new ctopics_delete();

// Page init
$topics_delete->Page_Init();

// Page main
$topics_delete->Page_Main();
...
var CurrentForm = ftopicsdelete = new ew_Form("ftopicsdelete", "delete");

// Form_CustomValidate event
ftopicsdelete.Form_CustomValidate = 
...
?>
<form name="ftopicsdelete" id="ftopicsdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($topics_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $topics_delete->Token ?>">
<?php } ?>
...
<?php if ($topics->id->Visible) { // id ?>
		<th class="<?php echo $topics->id->HeaderCellClass() ?>"><span id="elh_topics_id" class="topics_id"><?php echo $topics->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($topics->name_ar->Visible) { // name_ar ?>
		<th class="<?php echo $topics->name_ar->HeaderCellClass() ?>"><span id="elh_topics_name_ar" class="topics_name_ar"><?php echo $topics->name_ar->FldCaption() ?></span></th>
<?php } ?>
<?php if ($topics->name_en->Visible) { // name_en ?>
		<th class="<?php echo $topics->name_en->HeaderCellClass() ?>"><span id="elh_topics_name_en" class="topics_name_en"><?php echo $topics->name_en->FldCaption() ?></span></th>
<?php } ?>
<?php if ($topics->parent_id->Visible) { // parent_id ?>
		<th class="<?php echo $topics->parent_id->HeaderCellClass() ?>"><span id="elh_topics_parent_id" class="topics_parent_id"><?php echo $topics->parent_id->FldCaption() ?></span></th>
<?php } ?>
...
while (!$topics_delete->Recordset->EOF) {
	...
	// Set row properties
	$topics->ResetAttrs();
	$topics->RowType = EW_ROWTYPE_VIEW; // View
...
<?php if ($topics->id->Visible) { // id ?>
		<td<?php echo $topics->id->CellAttributes() ?>>
<span id="el<?php echo $topics_delete->RowCnt ?>_topics_id" class="topics_id">
...
<?php } ?>
<?php if ($topics->name_ar->Visible) { // name_ar ?>
		<td<?php echo $topics->name_ar->CellAttributes() ?>>
<span id="el<?php echo $topics_delete->RowCnt ?>_topics_name_ar" class="topics_name_ar">
...
<?php } ?>
<?php if ($topics->name_en->Visible) { // name_en ?>
		<td<?php echo $topics->name_en->CellAttributes() ?>>
<span id="el<?php echo $topics_delete->RowCnt ?>_topics_name_en" class="topics_name_en">
...
<?php } ?>
<?php if ($topics->parent_id->Visible) { // parent_id ?>
		<td<?php echo $topics->parent_id->CellAttributes() ?>>
<span id="el<?php echo $topics_delete->RowCnt ?>_topics_parent_id" class="topics_parent_id">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $topics_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftopicsdelete.Init();
