...
class Reviews extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // from_user_id
        $this->from_user_id = new DbField(
            $this, // Table
            'x_from_user_id', // Variable name
            'from_user_id', // Name
            '`from_user_id`', // Expression
            '`from_user_id`', // Basic search expression
            19, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`from_user_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->from_user_id->InputTextType = "text";
        $this->from_user_id->Nullable = false; // NOT NULL field
        $this->from_user_id->Required = true; // Required field
        $this->from_user_id->setSelectMultiple(false); // Select one
        $this->from_user_id->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->from_user_id->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->from_user_id->Lookup = new Lookup('from_user_id', 'users', false, 'id', ["name","type","",""], '', '', [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`name`, ''),'" . ValueSeparator(1, $this->from_user_id) . "',COALESCE(`type`,''))");
        $this->from_user_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->from_user_id->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['from_user_id'] = &$this->from_user_id;

        // to_user_id
        $this->to_user_id = new DbField(
            $this, // Table
            'x_to_user_id', // Variable name
            'to_user_id', // Name
            '`to_user_id`', // Expression
            '`to_user_id`', // Basic search expression
            19, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`to_user_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->to_user_id->InputTextType = "text";
        $this->to_user_id->Nullable = false; // NOT NULL field
        $this->to_user_id->Required = true; // Required field
        $this->to_user_id->Lookup = new Lookup('to_user_id', 'users', false, 'id', ["name","type","",""], '', '', [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`name`, ''),'" . ValueSeparator(1, $this->to_user_id) . "',COALESCE(`type`,''))");
        $this->to_user_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->to_user_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['to_user_id'] = &$this->to_user_id;

        // points
        $this->points = new DbField(
            $this, // Table
            'x_points', // Variable name
            'points', // Name
            '`points`', // Expression
            '`points`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`points`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->points->InputTextType = "text";
        $this->points->Nullable = false; // NOT NULL field
        $this->points->Required = true; // Required field
        $this->points->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->points->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        // review
        $this->review = new DbField(
            $this, // Table
            'x_review', // Variable name
            'review', // Name
            '`review`', // Expression
            '`review`', // Basic search expression
            201, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`review`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->review->InputTextType = "text";
        $this->review->Nullable = false; // NOT NULL field
        $this->review->Required = true; // Required field
        $this->review->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['review'] = &$this->review;
        ...
        $this->approved->Lookup = new Lookup('approved', 'reviews', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
        $this->approved->OptionCount = 2;
        $this->approved->DefaultErrorMessage = $Language->phrase("IncorrectField");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`reviews`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->from_user_id->DbValue = $row['from_user_id'];
        $this->to_user_id->DbValue = $row['to_user_id'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ReviewsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ReviewsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ReviewsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ReviewsAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ReviewsView", $parm);
        } else {
            $url = $this->keyUrl("ReviewsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ReviewsDelete");
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->to_user_id->EditValue = $this->to_user_id->CurrentValue;
        $this->to_user_id->PlaceHolder = RemoveHtml($this->to_user_id->caption());

        // points
        $this->points->setupEditAttributes();
        $this->points->EditValue = $this->points->CurrentValue;
        ...
        $this->review->EditValue = $this->review->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->from_user_id);
                    $doc->exportCaption($this->to_user_id);
                    $doc->exportCaption($this->points);
                    $doc->exportCaption($this->details);
                    $doc->exportCaption($this->review);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->from_user_id);
                        $doc->exportField($this->to_user_id);
                        $doc->exportField($this->points);
                        $doc->exportField($this->details);
                        $doc->exportField($this->review);
