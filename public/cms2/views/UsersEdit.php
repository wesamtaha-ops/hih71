...
loadjs.ready(["fusersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fusersedit", "x_birthday", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_users_country_id">
    <select
        ...
        data-select2-id="fusersedit_x_country_id"
        <?php } ?>
        data-table="users"
...
<script>
loadjs.ready("fusersedit", function() {
    var options = { name: "x_country_id", selectId: "fusersedit_x_country_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersedit.lists.country_id?.lookupOptions.length) {
        options.data = { id: "x_country_id", form: "fusersedit" };
    } else {
        options.ajax = { id: "x_country_id", form: "fusersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_users_currency_id">
    <select
        ...
        data-select2-id="fusersedit_x_currency_id"
        <?php } ?>
        data-table="users"
...
<script>
loadjs.ready("fusersedit", function() {
    var options = { name: "x_currency_id", selectId: "fusersedit_x_currency_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersedit.lists.currency_id?.lookupOptions.length) {
        options.data = { id: "x_currency_id", form: "fusersedit" };
    } else {
        options.ajax = { id: "x_currency_id", form: "fusersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("transfers", explode(",", $Page->getCurrentDetailTable())) && $transfers->DetailEdit) {
?>
