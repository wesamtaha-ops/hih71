...
<span id="el_transfers_currency_id">
    <select
        ...
        data-select2-id="ftransfersadd_x_currency_id"
        <?php } ?>
        data-table="transfers"
...
<script>
loadjs.ready("ftransfersadd", function() {
    var options = { name: "x_currency_id", selectId: "ftransfersadd_x_currency_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftransfersadd.lists.currency_id?.lookupOptions.length) {
        options.data = { id: "x_currency_id", form: "ftransfersadd" };
    } else {
        options.ajax = { id: "x_currency_id", form: "ftransfersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
