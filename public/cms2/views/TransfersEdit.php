...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="ftransfersedit" id="ftransfersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_transfers_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_transfers_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="transfers" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_transfers_user_id">
    <select
        ...
        data-select2-id="ftransfersedit_x_user_id"
        <?php } ?>
...
loadjs.ready("ftransfersedit", function() {
    var options = { name: "x_user_id", selectId: "ftransfersedit_x_user_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftransfersedit.lists.user_id?.lookupOptions.length) {
        options.data = { id: "x_user_id", form: "ftransfersedit" };
    } else {
        options.ajax = { id: "x_user_id", form: "ftransfersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_transfers_order_id">
    <select
        ...
        data-select2-id="ftransfersedit_x_order_id"
        <?php } ?>
        data-table="transfers"
...
<script>
loadjs.ready("ftransfersedit", function() {
    var options = { name: "x_order_id", selectId: "ftransfersedit_x_order_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftransfersedit.lists.order_id?.lookupOptions.length) {
        options.data = { id: "x_order_id", form: "ftransfersedit" };
    } else {
        options.ajax = { id: "x_order_id", form: "ftransfersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftransfersedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftransfersedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
