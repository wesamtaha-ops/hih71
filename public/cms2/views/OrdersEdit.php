...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fordersedit" id="fordersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<input type="hidden" data-table="orders" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_orders_student_id">
    <select
        ...
        data-select2-id="fordersedit_x_student_id"
        <?php } ?>
        data-table="orders"
...
loadjs.ready("fordersedit", function() {
    var options = { name: "x_student_id", selectId: "fordersedit_x_student_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fordersedit.lists.student_id?.lookupOptions.length) {
        options.data = { id: "x_student_id", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_student_id", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_orders_teacher_id">
    <select
        ...
        data-select2-id="fordersedit_x_teacher_id"
        <?php } ?>
        data-table="orders"
...
<script>
loadjs.ready("fordersedit", function() {
    var options = { name: "x_teacher_id", selectId: "fordersedit_x_teacher_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fordersedit.lists.teacher_id?.lookupOptions.length) {
        options.data = { id: "x_teacher_id", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_teacher_id", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_orders_topic_id">
    <select
        ...
        data-select2-id="fordersedit_x_topic_id"
        <?php } ?>
        data-table="orders"
...
<script>
loadjs.ready("fordersedit", function() {
    var options = { name: "x_topic_id", selectId: "fordersedit_x_topic_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fordersedit.lists.topic_id?.lookupOptions.length) {
        options.data = { id: "x_topic_id", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_topic_id", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
loadjs.ready(["fordersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fordersedit", "x_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_orders_currency_id">
    <select
        ...
        data-select2-id="fordersedit_x_currency_id"
        <?php } ?>
        data-table="orders"
...
<script>
loadjs.ready("fordersedit", function() {
    var options = { name: "x_currency_id", selectId: "fordersedit_x_currency_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fordersedit.lists.currency_id?.lookupOptions.length) {
        options.data = { id: "x_currency_id", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_currency_id", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_orders_status">
    <select
        ...
        data-select2-id="fordersedit_x_status"
        <?php } ?>
        data-table="orders"
        data-field="x_status"
...
<script>
loadjs.ready("fordersedit", function() {
    var options = { name: "x_status", selectId: "fordersedit_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fordersedit.lists.status?.lookupOptions.length) {
        options.data = { id: "x_status", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_status", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
