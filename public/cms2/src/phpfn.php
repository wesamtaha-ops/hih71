...
function GetMultiSearchSqlFilter($fldExpression, $fldOpr, $fldVal, $dbid, $sep)
{
    ...
    $sql .= $fldExpression . Like(QuotedValue($fldVal . $sep, DATATYPE_STRING, $dbid, "STARTS WITH"), $dbid) . " OR " .
        $fldExpression . Like(QuotedValue($sep . $fldVal . $sep, DATATYPE_STRING, $dbid, "LIKE"), $dbid) . " OR " .
        $fldExpression . Like(QuotedValue($sep . $fldVal, DATATYPE_STRING, $dbid, "ENDS WITH"), $dbid);
...
function SearchFilter($fldExpression, $fldOpr, $fldVal, $fldType, $dbid)
{
    ...
    if (in_array($fldOpr, ["=", "<>", "<", "<=", ">", ">="])) {
        ...
        $filter .= Like(QuotedValue($fldVal, DATATYPE_STRING, $dbid, $fldOpr), $dbid);
    } elseif (in_array($fldOpr, ["NOT STARTS WITH", "NOT LIKE", "NOT ENDS WITH"])) {
        $filter .= NotLike(QuotedValue($fldVal, DATATYPE_STRING, $dbid, $fldOpr), $dbid);
...
function QuotedValue($value, $fldType, $dbid = 0, $likeOpr = "")
{
    if ($value === null) {
        return "NULL";
    }
        ...
        foreach ($ar as $keyword) {
            if ($keyword != "") {
                ...
                if ($keyword == "OR" && $searchType == "") {
                    ...
                    $wrk = $fld->VirtualExpression . Like(QuotedValue($keyword, DATATYPE_STRING, $dbid, "LIKE"), $dbid);
                } elseif ($fld->DataType != DATATYPE_NUMBER || is_numeric($keyword)) {
                    $wrk = $fld->BasicSearchExpression . Like(QuotedValue($keyword, DATATYPE_STRING, $dbid, "LIKE"), $dbid);
                }
                if ($wrk != "") {
                    $arSql[$j] = $wrk;
                    $arCond[$j] = $defCond;
...
function GetExtendedFilter(&$fld, $default = false, $dbid = 0)
{
    ...
    $wrk = "";
    if (in_array($fldOpr, ["BETWEEN", "NOT BETWEEN"])) {
        $isValidValue = $fldDataType != DATATYPE_NUMBER || $fld->VirtualSearch || IsNumericSearchValue($fldVal, $fldOpr, $fld) && IsNumericSearchValue($fldVal2, $fldOpr2, $fld);
        if ($fldVal != "" && $fldVal2 != "" && $isValidValue) {
