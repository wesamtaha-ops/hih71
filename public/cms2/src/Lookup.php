...
class Lookup
{
    ...
    {
        return $this->getSearchExpression() . Like(QuotedValue($sv, DATATYPE_STRING, $this->Table->Dbid, "STARTS WITH"), $this->Table->Dbid);
    }
    ...
    protected function getModalSearchFilter($sv, $dbid)
    {
        ...
        foreach ($ar as $keyword) {
            if ($keyword != "") {
                $thisFilter = $this->getSearchExpression() . Like(QuotedValue($keyword, DATATYPE_STRING, $dbid, "LIKE"), $dbid);
